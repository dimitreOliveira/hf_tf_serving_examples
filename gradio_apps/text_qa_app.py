...
server_port = os.environ.get("SERVER_PORT", 7863)
...
def preprocess(question, text, tokenizer):
    return tokenizer(question, text)

def postprocess(start, end, input_ids, tokenizer):
...
def predict(question, text):
    ...
    answer = postprocess(prediction["start"], prediction["end"], 
                        tokenized_inputs.input_ids, tokenizer)
    return answer

iface = gr.Interface(
    fn=predict,
    inputs=[gr.inputs.Textbox(label="Question", placeholder="Input text question here..."), 
