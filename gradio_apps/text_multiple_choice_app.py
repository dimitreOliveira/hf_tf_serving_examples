...
rest_url = os.environ.get("TF_URL", "http://localhost:8501/v1/models/multiple_choice:predict")

print(f'Requesting predictions from: "{rest_url}"')
print(f'Loading tokenizer from: "{tokenizer_path}"')
tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_path)

def preprocess(prompt, choice1, choice2, tokenizer):
    return tokenizer([prompt, prompt], [choice1, choice2], padding=True)

def predict(prompt, choice1, choice2):
...
iface = gr.Interface(
    fn=predict,
    inputs=[gr.inputs.Textbox(label="Prompt", placeholder="Input text prompt here..."), 
