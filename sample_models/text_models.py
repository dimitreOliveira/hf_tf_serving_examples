...
def get_distilbert_embedding(
    tokenizer_output: str = "./tokenizers",
    model_output: str = "./models/embedding",
...
def get_distilbert_custom(
    tokenizer_output: str = "./tokenizers",
    model_output: str = "./models/custom",
    model_version: str = "1",
    model_name: str = "distilbert-base-uncased",
):
    tokenizer = transformers.DistilBertTokenizer.from_pretrained(model_name)
    ...
    # Model architecture
    inputs = {
        layer: L.Input(shape=(None,), dtype=tf.int32, name=layer)
        for layer in tokenizer.model_input_names
    }
